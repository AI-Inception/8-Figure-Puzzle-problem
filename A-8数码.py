...
closelist_contrast=[]
if Solve(A,B): 
    final=Search(init,algorithm='A*-h2')
    print('搜索得到的最短路径为: ')
    printpath(final)
...
list1=[]
list2=[]
for i in closelist_contrast:
    for j in closelist_contrast:
...
nx.draw(G, with_labels=True, font_size=max(1*len(list1),20),node_size=250*len(list1), node_color="skyblue",node_shape='s', alpha=0.8, linewidths=len(list1))
plt.savefig('searchTree.jpg')
print('请打开searchTree.jpg查看A*-h2绘制的搜索树')


# In[44]:


is_contrast=int(input('是否查看四种搜索算法对比，是输入1，否输入0:'))
if is_contrast:
    print('搜索算法对比')
    print('-----------------------------------------------')
    openlist_contrast=[]
